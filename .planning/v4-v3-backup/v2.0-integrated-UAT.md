...
- truth: "Deploy output shows source file paths (SSOT provenance)"
  ...
  severity: minor
  test: 12
  root_cause: "All deploy echo statements use only display name ($name), not source path. Source paths are available in variables ($source, $PLUGIN_DIR/hooks/$hook etc.) but never printed. backup_and_link() in lib/symlinks.sh also only prints $name."
  artifacts:
    - path: "plugins/claude/deploy.sh"
      issue: "~8 echo statements use hardcoded basenames instead of source path variables"
    - path: "lib/symlinks.sh"
      issue: "backup_and_link() lines 111/118/126/152 print $name only, not $src → $dest"
  missing:
    - "Include source path in all deploy status messages"
    - "Update backup_and_link() to show src → dest mapping"
  debug_session: ""
...
- truth: "Project-configs wizard is interactive with meaningful scaffolding output"
  ...
  severity: major
  test: 13
  root_cause: "Three issues: (1) wizard_yesno function not available in project.sh execution context — functions from lib/wizard.sh not reaching the plugin, causing prompts to fall through or error. (2) CLAUDE.md builder (lines 388-407) creates hardcoded boilerplate instead of calling _claude_build_md() to assemble from user's configured sections (CLAUDE_MD_SECTIONS). (3) templates/settings/hooks.json contains $CLAUDE_PROJECT_DIR/plugins/claude/hooks/ path but hooks are deployed to .claude/hooks/ in target project."
  artifacts:
    - path: "plugins/claude/project.sh"
      issue: "wizard_yesno calls at lines 221/230/276/314/388/428/506 fail silently; CLAUDE.md builder at 388-407 is hardcoded boilerplate; hooks.json template has wrong paths"
    - path: "plugins/claude/templates/settings/hooks.json"
      issue: "Hook command paths point to dotconfigs source repo, not deployed .claude/hooks/ location"
  missing:
    - "Ensure wizard functions are available in project.sh context"
    - "Replace boilerplate CLAUDE.md with _claude_build_md() assembly from global sections"
    - "Fix hooks.json template to use .claude/hooks/ paths"
  debug_session: ""
