...
must_haves:
  ...
  artifacts:
    - path: "lib/config.sh"
      provides: "Shared config loading helper and SSOT variable reference"
    ...
    - path: "plugins/git/templates/git-hooks.conf"
      provides: "Git plugin config template with all GIT_HOOK_* defaults"
      contains: "GIT_HOOK_BLOCK_AI_ATTRIBUTION"
    - path: "plugins/claude/templates/claude-hooks.conf"
      provides: "Claude plugin config template with CLAUDE_HOOK_* settings"
      contains: "CLAUDE_HOOK_RUFF_FORMAT"
  key_links:
    - from: "lib/config.sh"
      to: "plugins/git/hooks/commit-msg"
      via: "SSOT variable naming reference (documentation, not runtime sourcing — hooks use inline config loading)"
      pattern: "GIT_HOOK_"
    - from: "plugins/git/templates/git-hooks.conf"
      to: "plugins/git/hooks/commit-msg"
      via: "deployed config file sourced by hooks at runtime"
      pattern: "GIT_HOOK_BLOCK_AI_ATTRIBUTION"
---
...
<task type="auto">
  <name>Task 3: Refactor pre-push hook, create plugin config templates, remove hooks.conf profiles</name>
  <files>plugins/git/hooks/pre-push, plugins/git/templates/git-hooks.conf, plugins/claude/templates/claude-hooks.conf, plugins/claude/templates/hooks-conf/default.conf, plugins/claude/templates/hooks-conf/strict.conf, plugins/claude/templates/hooks-conf/permissive.conf</files>
  <action>
...
**Create git plugin config template (`plugins/git/templates/git-hooks.conf`):**
Per locked decision: "Git-related settings move to git plugin templates." Create a bash-sourceable config template containing all GIT_HOOK_* defaults:
```bash
# Git hook configuration — deployed by dotconfigs project
# Override these values per-project. Precedence: this file > global .env > defaults below.
...
GIT_HOOK_MAX_SUBJECT_LENGTH=72
...
GIT_HOOK_BRANCH_INFO=true
```
This template is deployed to the user-selected config path during `dotconfigs project` (Plan 05). Hooks source this file at runtime.

**Create claude plugin config template (`plugins/claude/templates/claude-hooks.conf`):**
Per locked decision: "Claude-related settings stay in claude plugin templates." Create a bash-sourceable config template:
```bash
...
CLAUDE_HOOK_FILE_PROTECTION=true
```
This template is deployed to `.claude/claude-hooks.conf` during `dotconfigs project claude` (Plan 05).
...
The hooks.conf profile concept is replaced by per-setting toggles in the wizard (Plan 05). Git hook config lives in git plugin's domain, claude-specific config stays in claude plugin — each plugin deploys its own config files.
  </action>
  <verify>
...
- `test -f plugins/git/templates/git-hooks.conf` confirms git config template exists
- `grep -c 'GIT_HOOK_' plugins/git/templates/git-hooks.conf` returns 15+ (all git hook vars)
- `test -f plugins/claude/templates/claude-hooks.conf` confirms claude config template exists
- `grep 'CLAUDE_HOOK_RUFF_FORMAT' plugins/claude/templates/claude-hooks.conf` finds claude setting
...
- `grep -E 'local -n|declare -n' plugins/git/hooks/pre-push` returns nothing
  ...
  <done>pre-push uses unified GIT_HOOK_BRANCH_PROTECTION naming with metadata header. Git plugin config template created at plugins/git/templates/git-hooks.conf with all GIT_HOOK_* defaults. Claude plugin config template created at plugins/claude/templates/claude-hooks.conf with all CLAUDE_HOOK_* settings. hooks.conf profile templates deleted.</done>
</task>
