...
//
// See: global.json (actual config, SSOT for all groups/modules)
{
  "claude": {
    "hooks": {
...
// Generated by `dotconfigs project-init` from global.json (SSOT).
// Known groups (claude, git) get curated project-specific overrides.
// Other groups get auto-transformed targets (tilde stripped).
{
  "claude": {
    "hooks": {
      "source": "plugins/claude/hooks",
