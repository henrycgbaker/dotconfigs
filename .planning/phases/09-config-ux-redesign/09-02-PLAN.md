...
files_modified:
  ...
  - .gitignore
autonomous: true
# Note: Task 2 (wizard helpers) and Task 3 (template + gitignore) split for scope clarity
...
Output: Extended lib/colours.sh (Task 1), extended lib/wizard.sh (Task 2), settings.json template + .gitignore (Task 3).
</objective>
...
<task type="auto">
  <name>Task 2: Add opt-in wizard helpers to lib/wizard.sh</name>
...
<task type="auto">
  <name>Task 3: Create settings.json template + add .gitignore entry</name>
  <files>plugins/claude/templates/settings/settings-template.json, .gitignore</files>
  <action>
**Part A: Create complete settings.json template**

Create `plugins/claude/templates/settings/settings-template.json` (inside the existing `settings/` directory alongside base.json, python.json, etc.) as a complete working example with ALL common rules. This is the public template (users delete what they don't want). Include:
...
**Part B: Add settings.json to .gitignore**
...
Read the existing `.gitignore` file. Add `settings.json` entry if not already present. This prevents the personal root `settings.json` (which contains user-specific rules like nvidia-smi, docker, /proc/* reads) from being committed when the repo goes public.
  ...
  <verify>
- `test -f plugins/claude/templates/settings/settings-template.json && echo "exists"`
- `python3 -c "import json; json.load(open('plugins/claude/templates/settings/settings-template.json'))" && echo "valid JSON"`
- `grep -q "settings.json" .gitignore && echo "gitignored"`
  </verify>
  <done>
- Complete settings.json template exists at plugins/claude/templates/settings/settings-template.json
- Root settings.json is listed in .gitignore
  </done>
</task>
