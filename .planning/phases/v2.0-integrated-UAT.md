...
## Current Test
<!-- Automated checks done, awaiting manual verification from test 12 onwards -->

number: 12
name: Deploy Idempotency
...
result: [pending manual] (automated: second deploy shows all Unchanged, --force resolves conflicts, content-change detection works)
...
result: [pending manual] (automated: wizard starts, shows [G] badge, detects brownfield)
...
result: [pending manual] (automated: 7 hooks present, permissions 755)
...
result: [pending manual] (automated: dots → dotconfigs in repo, PATH symlinks at ~/.local/bin/, identical output)
