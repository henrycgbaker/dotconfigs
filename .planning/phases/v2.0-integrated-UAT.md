...
## Current Test
<!-- Re-testing in logical user-journey order: setup → global-configs → deploy → status → project -->

number: 12
name: Deploy Idempotency
expected: |
  Run `./dotconfigs deploy` twice. Second run shows "Unchanged" counts —
  no errors, no re-creation of existing files. Safe to run repeatedly.
...
total: 17
passed: 6
issues: 5 (3 major fixed via quick-003, 2 blockers fixed inline)
pending: 6
skipped: 0
