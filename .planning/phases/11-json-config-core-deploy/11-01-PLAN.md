...
5. `deploy_module()` -- Takes source, target, method, include_csv, dotconfigs_root, dry_run (true/false), interactive_mode (true/false/force). Steps:
   ...
   - Switch on method:
     - **symlink**: If source is a directory, call `deploy_directory_files`. If source is a file, call `backup_and_link` from lib/symlinks.sh (for live deploy) or print "Would link" (for dry-run).
...
6. `deploy_directory_files()` -- Takes source_dir, target_dir, include_csv, dotconfigs_root, dry_run, interactive_mode. Steps:
   ...
   - For each file, call `backup_and_link` (or dry-run equivalent)
