...
Check current `plugins/claude/settings.json` content. The hook paths should already use `~/.claude/hooks/` absolute paths (from Phase 10 fix). If they still contain `$CLAUDE_PROJECT_DIR`, replace with absolute paths to `~/.claude/hooks/`.
